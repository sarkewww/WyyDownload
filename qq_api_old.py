...
class QQMusicAPI:
    ...
    def get_song_url(self, songmid: str, quality: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {
            'req_1': {
                ...
                'param': {
                    'filename': [file_name], 'guid': self.guid,
                    'songmid': [songmid], 'songtype': [0],
                    'uin': self.uin, 'loginflag': 1, 'platform': '20'
                }
            },
            'loginUin': self.uin,
    ...
    def get_song_detail(self, songmid: str, songid: int = 0) -> Dict[str, Any]:
        if not songid and songmid and songmid.isdigit():
            songid = int(songmid)
            songmid = ''
        if songid:
        ...
        try:
            ...
            return {
                'songs': [{
                    ...
                    'al': {'name': album_info.get('name', ''), 'picUrl': pic_url, 'mid': album_mid},
    ...
    def get_lyric(self, songid: int, cookies: Dict[str, str] = None) -> Dict[str, Any]:
        payload = {
            'music.musichallSong.PlayLyricInfo.GetPlayLyricInfo': {
                ...
                'param': {'trans_t': 0, 'roma_t': 0, 'crypt': 0, 'lrc_t': 0,
                          'interval': 208, 'trans': 1, 'ct': 6, 'songID': songid}
            },
            'comm': {'ct': '6', 'cv': '80600'}
        }
        try:
            response_text = self._request(QQAPIConstants.BASE_URL, json.dumps(payload))
            data = json.loads(response_text)
            ld = data.get('music.musichallSong.PlayLyricInfo.GetPlayLyricInfo', {}).get('data', {})
            lyric = base64.b64decode(ld['lyric']).decode('utf-8') if ld.get('lyric') else ''
            tlyric = base64.b64decode(ld['trans']).decode('utf-8') if ld.get('trans') else ''
            return {'lrc': {'lyric': lyric}, 'tlyric': {'lyric': tlyric}, 'code': 200}
        except Exception as e:
            self.logger.error(f"获取QQ歌词失败: {e}")
            return {'lrc': {'lyric': ''}, 'tlyric': {'lyric': ''}, 'code': 200}

    def process_request(self, song_url: str) -> Dict[str, Any]:
        songmid = self._extract_songmid(song_url)
        if not songmid:
            return {'error': '歌曲ID无效'}
        ...
        info = self.get_song_detail(mid, sid)
        if not info or not info.get('songs'):
            return {'error': '歌曲不存在或获取信息失败'}

        song = info['songs'][0]
        real_mid = song.get('mid', mid)
        real_id = song.get('id', str(sid))

        lyric = self.get_lyric(int(real_id))

        file_types = ['aac_48', 'aac_96', 'aac_192', 'ogg_96', 'ogg_192',
                      'ogg_320', 'ogg_640', 'atmos_51', 'atmos_2',
                      'master', 'flac', '320', '128']
        urls = {}
        for ft in file_types:
            r = self.get_song_url(real_mid, ft)
            if r and r.get('url'):
                urls[ft] = {'url': r['url'], 'bitrate': r['bitrate'], 'ext': r.get('ext', '')}

        singers = '/'.join(a['name'] for a in song.get('ar', []))
        return {
            'music_info': {
                'id': real_id, 'mid': real_mid,
                'name': song['name'], 'singer': singers,
                'album': (song.get('al') or {}).get('name', ''),
                'pic': (song.get('al') or {}).get('picUrl', ''),
                'interval': song.get('dt', 0) // 1000,
            },
            'music_url': urls,
            'music_lyric': lyric,
        }
